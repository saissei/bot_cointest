...
;(async () => {
  try {
    const reqBody = {
      userId: process.env.user002UserId,
      customerId: process.env.user002CustomerId,
      password: process.env.user002Password
    }
    console.log(reqBody)
...
;(async () => {
  try {
    const jwt = process.env.adminJwt
